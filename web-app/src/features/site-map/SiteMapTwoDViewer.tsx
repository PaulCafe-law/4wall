...
export function SiteMapTwoDViewer({
  ...
  return (
    <div className="relative min-h-[34rem] overflow-hidden rounded-[1.5rem] border border-chrome-200 bg-chrome-100">
      ...
      <div className="absolute bottom-4 left-4 rounded-2xl border border-white/70 bg-white/85 px-4 py-3 text-xs leading-5 text-chrome-700 shadow-panel">
        {siteLabel} 2D 座標優先使用 floorplanX / floorplanY；缺少座標時使用固定示意位置。
      </div>
      {markers.map((marker) => (
        <SiteMapMarkerButton
